...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size):
        ...
        self.conv = nn.Conv1d(input_size, hidden_size, kernel_size=1)  # 一维卷积
...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden):
        ...
        attns = torch.bmm(v, torch.tanh(torch.bmm(W, hidden)))  # 对应attention中的公式uij = Vt tanh(W1ej+W2di)
...
class Pointer(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden, last_hh):
        ...
        context = enc_attn.bmm(static_hidden.permute(0, 2, 1))  # (B, 1, num_feats) 中间向量
        ...
        probs = torch.bmm(v, torch.tanh(torch.bmm(W, energy))).squeeze(1)  # 对应Pointer Network的公式p(Ci) = softmax(ui)
...
class DRL4TSP(nn.Module):
    ...
    def forward(self, static, dynamic, decoder_input=None, last_hh=None):
        ...
        # shape [256,128,21] batch_size:256  hidden_dim:128  node_num:21
